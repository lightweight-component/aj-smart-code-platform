...
public class PageEnhancer {
    ...
    public void getParams(HttpServletRequest req) {
        if (req == null) {// 可能在测试
            start = 0;
            limit = Pager.DEFAULT_PAGE_SIZE;

            return;
        }

        Integer pageSize = get(req, PAGE_SIZE);
        limit = pageSize == null ? Pager.DEFAULT_PAGE_SIZE : pageSize;
...
//            pageSql = selectStatement.toString();
            if (jdbcReader.getDatabaseVendor() == JdbcConstants.DatabaseVendor.MYSQL)
                pageSql = sql + " LIMIT " + start + ", " + limit;
            else if (jdbcReader.getDatabaseVendor() == JdbcConstants.DatabaseVendor.DERBY)
                pageSql = sql + " OFFSET " + start + " ROWS FETCH NEXT " + limit + " ROWS ONLY";
            else throw new RuntimeException("TODO: add db vendor");
